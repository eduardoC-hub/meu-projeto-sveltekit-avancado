import {fail, redirect} from '@sveltejs/kit';


export const actions = {
    guerreiros: async ({request}) => {
    const data = await request.formData();
    const nascimento = data.get('nascimento');
    const forca = Number(data.get('forca'));

    if (forca < 7) return fail(400, {error: 'Força insuficiente para ser um guerreiro.'});
    const atual = new Date(), aniversario = new Date(nascimento);
    const idade = atual - aniversario;
    if (idade < 568036800000) return fail(400, {error: 'precisa de ao menos 16 anos'});
    redirect(303, '/07/irmandade/guerreiros');
    },
    magos: async ({request}) => {
    const data = await request.formData();
    const nascimento = data.get('nascimento');
    const inteligencia = Number(data.get('inteligencia'));
    const magia = data.get('magia');

    if (inteligencia < 8) return fail(400, {error: 'Inteligência insuficiente para ser um mago.'});
    if (magia != 'on') return fail(400, {error: 'Magia insuficiente para ser um mago.'});
    const atual = new Date(), aniversario = new Date(nascimento);

    const idade = atual - aniversario;
    if (idade < 504921600000) return fail(400, {error: 'precisa de ao menos 16 anos'});
    redirect(303, '/07/irmandade/magos');
},
    artesaos: async ({request}) => {
    const data = await request.formData();
    const nascimento = data.get('nascimento');
    const destreza = Number(data.get('destreza'));
    const ferramentas = data.get('ferramentas');
    if (destreza < 6) return fail(400, {error: 'te falta destreza meu jovem!'});
    if (ferramentas != 'on') return fail(400, {error: 'Habilidade insuficiente para ser um artesão.'});
    const atual = new Date(), aniversario = new Date(nascimento);
    const idade = atual - aniversario;
    if (idade < 473364000000) return fail(400, {error: 'precisa de ao menos 16 anos'});
    redirect(303, '/07/irmandade/artesaos');
},
};
